...
def convert(in_path, out_path: Path) -> None:
    ...
    if key is not None:
        ...
        for region_id in set(mesh.cell_data_dict[key]['tetra']):
            surface_points, surface_faces = _surface_from_mesh(mesh.points, mesh.cells_dict['tetra'][mesh.cell_data_dict[key]['tetra'] == region_id,:])

            out_tris = np.vstack([out_tris, surface_faces + out_points.shape[0]])
            out_points = np.vstack([out_points, surface_points])
    else:
        out_points, out_tris = _surface_from_mesh(mesh.points, mesh.cells_dict['tetra'])
...
def _surface_from_mesh(points, tets: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    surface_point_id = np.unique(boundary_tris.ravel())

    lut = np.full(surface_point_id.max()+1, 0, dtype=_smallest_uint_dtype(surface_point_id.size-1))
